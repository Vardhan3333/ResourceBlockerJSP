...
<%if(resource.size()==0)  { %>
<div><p>No Selection Found</p></div>
<table class="table">
  ...
  <tbody>
    ...
    <form action="/blocking" method="get" onSubmit="return click1()" >
    <input type="hidden" value="<%=resource.get(i).getRid() %>" name="id"/>
    		
    		<td><input type="submit" value="Block" /><td>
    <script>
      function click1(){
      var v = document.getElementById("rid").value;
      if(confirm("Confirm Blocking Resource :" + v)){
      return true;
      }
      else{
      return false;
